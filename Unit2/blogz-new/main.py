...
def display_blog_entries():
    ...
    # entry_owner_id = request.args.get('entries')
    # owner_id = request.args.get('owner.id')
    # owner_email = request.args.get('entry.owner.email')
    if (entry_id):
    ...
    if user_id:
        ...
        user_posts = user.entries
        return render_template('user_blogs.html', title='user blogs', user=user, user_posts=user_posts)



    # if (owner_id):
    #     entries = Entry.query.filter_by(owner_id = owner_id).all()
    #     return render_template('user_blogs.html', title="Author Blog Entries", entries = entries)
...
# @app.route("/blog?owner={{ entry.owner.id }}", methods=['POST', 'GET'])
# def blog_owner_list():
#     blog_id = request.args.get('id')
#
#     if blog_id:
#         entries = Entry.query.filter_by(id=blog_id).all()
#         selected_post = Entry.query.filter_by(id=blog_id).first()
#         return render_template("blogs.html", title=selected_post.title, entries=entries)
#
#
#
#     user_email = request.args.get('email')
#     if user_email:
#         entries = Entry.query.filter_by(owner_id=user_email).order_by(Entry.created.desc()).all()
#         if len(entries) > 0:
#             return render_template("user_blogs.html", title="{}'s Posts".format(entries[0].owner.email), entries=entries)
#         else:
#             return render_template("user_blogs.html", title="This user has no entries yet.", entries=entries)
