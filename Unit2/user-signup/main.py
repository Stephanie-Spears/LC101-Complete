...
def index():
    ...
    if request.method == 'POST':
        ...
        if not password or not re.match("^[a-zA-Z0-9_]{3,20}$", password):
            ...
            return render_template("index.html", error=error)
        if (not password or not re.match("^[a-zA-Z0-9_]{3,20}$", password)) and (not username or not re.match("^[a-zA-Z0-9_]{3,20}$", username)):
            error = "Password error and username error"
            return render_template("index.html", error=error)
